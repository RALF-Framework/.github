...
Review the core meta-model.
Explore an example domain pack.
Create a small lifecycle for your own organization or team.
Define roles, artifacts, gates, and knowledge sources.
Generate or manually write a task context packet.
Validate the model with real users before automating anything.
...
phases:
  ...
  - id: triage
    name: Triage
    owner\_role: maintenance\_planner
    gates:
      - safety\_check
  - id: execute
    name: Execute Work
    owner\_role: technician
    output\_artifacts:
      - work\_log
  - id: close
    name: Close and Learn
    owner\_role: maintenance\_lead
    output\_artifacts:
      - closure\_report
```
---
...
Lifecycle before agents — understand the work before assigning automation.
Humans remain accountable — AI can assist, but ownership and risk stay with people and organizations.
Knowledge is first-class — standards, policies, examples, and lessons learned are part of the model.
Every task produces or transforms artifacts — outputs should be durable, reviewable, and traceable.
Every high-risk action needs a gate — approvals and validations must be explicit.
Tools use least privilege — agents and workflows should only access what they need.
Context is compiled — each task receives bounded context, not the entire organization.
Runtime adapters are replaceable — RALF should work across different workflow and agent systems.
Domain packs are reusable — common operating patterns should be packaged, versioned, and improved.
Traceability builds trust — decisions, artifacts, approvals, and tool actions should be auditable.
---
RALF and AI agents
